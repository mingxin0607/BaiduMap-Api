...
raw_data_residential = pd.read_excel(file_path_residential, header=0)  # header=0表示第一行是表头，就自动去除了
...
raw_data_facility = pd.read_excel(file_path_facility, header=0)  # header=0表示第一行是表头，就自动去除了
...
for i in range(len(residential_indexs)):
    if (exit_flag): 
        break
    ...
    for j in range(len(facility_indexs)):
        ...
        if (min_dist > straight_distance):
            min_dist = straight_distance
            min_j = j
        
    try:
        # 找路
        t1 = facility_lats[min_j]
        t2 = facility_lons[min_j]
        route = find_route(s1,s2,t1,t2)
        api_count += 1
        # print(route)
        
        # 处理路线信息
        res_vec.append(route["result"]["routes"][0]["distance"])
...
# 计算平均值
average = sum(res_vec) / len(res_vec)

# 计算标准差
import math
std_deviation = math.sqrt(sum((x - average) ** 2 for x in res_vec) / len(res_vec))

# 输出结果
print("平均值: ", average)
print("标准差: ", std_deviation)
print("变异系数: ", std_deviation / average)